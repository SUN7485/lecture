...
(function () {
  ...
  class LectureEditor {
    ...
    _buildOverlay() {
      ...
      } else if (this._canFloat(this.selected.closest('.ve-slot') || this.selected)) {
        // Text/box blocks get side handles only: pull an edge to reflow the
        // text narrower or wider (e.g. to clear space for an image beside it).
        ['e','w'].forEach(pos => {
          const h = doc.createElement('div');
          h.className = 've-handle ' + pos;
          h.dataset.pos = pos;
      ...
      if (isImg) {
        ...
        this.toolbar.innerHTML = `
          ...
          ${wandBtn}
          ${editBtn}
          <button data-act="fontup" title="Bigger text">A+</button>
          <button data-act="fontdn" title="Smaller text">A−</button>
      ...
      this.toolbar.addEventListener('click', (e) => {
        ...
        else if (act === 'fontup' || act === 'fontdn') this._bumpFont(act === 'fontup' ? 1 : -1);
    ...
    _startResize(e, pos) {
      ...
      const floated = unit.dataset.veFloat === '1';
      const baseLeft = parseFloat(unit.style.left) || 0;
      ...
      // Text reflows on its own — never freeze a text block's height.
      if (kind === 'x' && isImg) img.style.height = Math.round(startH) + 'px';
      if (kind === 'y' && isImg) img.style.width = Math.round(startW) + 'px';
      ...
      const move = (ev) => {
        if (kind === 'corner') {
          ...
          if (!isImg) {
            ...
            // West-edge resize on a floated block: keep the right edge
            // anchored by shifting the box as the width changes.
            if (dirX === -1 && floated) {
              unit.style.left = Math.round(baseLeft + (startW - w)) + 'px';
            }
          }
        } else {
    ...
    // ---------- font size (A+ / A− on the text toolbar) ----------
    _bumpFont(dir) {
      const el = this.selected;
      if (!el) return;
      const cur = parseFloat(this.win.getComputedStyle(el).fontSize) || 16;
      const next = Math.max(8, Math.min(200, Math.round(cur * (dir > 0 ? 1.1 : 1 / 1.1))));
